...
import {HOME_ROUTE, PROJECTS_ROUTE, SIGN_IN_ROUTE} from "../../utils/const";
        ...
        <header className="absolute inset-x-0 top-0 z-50">
            <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
                ...
                <div className="hidden lg:flex lg:gap-x-12">
                    {navigation.map((item) => (
                        <a key={item.name} onClick={() => menuHandle(item.name)} className="text-sm/6 cursor-pointer font-semibold text-gray-900">
                            {item.name}
                        </a>
                    ))}
                </div>
